...
name: football-data
description: >
  Football (soccer) standings, schedules, match statistics, xG, transfers,
  profiles, head-to-head history, Elo strength, and forecasts across major
  leagues. Use for football fixtures, form, lineups, injuries, team or player
  research, and model context. Data updates post-match, so do not use it as a
  live-score or executable-odds source; use read-only market skills for current
  prices. Use the corresponding sport-specific skill for non-football sports.
  Premier League-only endpoints and top-five-league xG limits remain documented
  in the body and API reference.
license: MIT
metadata:
